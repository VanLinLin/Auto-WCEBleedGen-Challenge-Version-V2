...
class COCOeval:
    ...
    def _prepare(self):
        ...
        if p.useCats:
            ...
            # f_gts = open('./gts_catid.json','w+')
            # json_gt = json.dumps(gts)
            # f_gts.write(json_gt)
            # f_gts.close()

            # f_dts = open('./dts_catid.json','w+')
            ...
            # f_gts = open('./gts_no_catid.json','w+')
            # json_gt = json.dumps(gts)
            # f_gts.write(json_gt)
            # f_gts.close()

            # f_dts = open('./dts_no_catid.json','w+')
            # json_dt = json.dumps(dts)
            # f_dts.write(json_dt)
            # f_dts.close()
        ...
        for k, k0 in enumerate(k_list):  ##类别的索引下标遍历
            ...
            for a, a0 in enumerate(a_list):  ##aRng的遍历
                ...
                for m, maxDet in enumerate(m_list):
                    ...
                    unique_id = np.array([np.unique(i[indice_gt[p]]) for p,i in enumerate(gtmatch_id)],dtype=object)  #(T,d（不同的iou下，maxDet下的预测框能检测到的gt，所以维度d维度不一样）)
    ...
    def get_good_predict_data(self):
        ...
        get_imgid_excellent_predict(save_path, iouThr=.5, areaRng='all', maxDets=self.params.maxDets[0], catId=[self.params.catIds[0]])
